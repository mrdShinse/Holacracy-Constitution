...
それは一般的な状況での役割の説明責任と権限を制定すべきである誰の透明度を低下しないようにリードのリンクであれば、同じ定義された役割に複数の人を割り当てることができます。
...
サークルは、その選出役割に説明責任またはドメインを追加するだけでなく、修正またはそれらの追加を除去することができます。
...
処理のために有効であるために、提案は解決するか、提案者によって感知されたテンションを減らす必要があります。また、提案は通常、提案者がより良い目的やサークルでの提案者の役割の1の責任を表現する助けなければなりません。しかし、その代わりに別のサークルのメンバーを助けるかもしれない提案は、より良いサークルでその人の役割のいずれかを発現するが、場合にのみ、その人がその役割を表現するために提案者権限を付与しています。
...
- **（e）の**処理や提案を採用することは、この憲法に定義されたルールを破る、またはこの憲法の下で付与された権限の外に行動するサークルやそのメンバーを促します。関係する誰もがこれらの出力を制定するための提案は、憲法の規則に違反することが異議を上げることができるので、例えば、次に、アクション、プロジェクト、および特定の業務の決定は、通常、3.1節の条件に即し有効なガバナンス出力ではありません。
...
プロセスの内訳は、合理的な期間内に解決されない場合は、その後、最終的に問題のあるサークルが含まれている任意のスーパーサークルのファシリテーターは、同様に問題のあるサークルのスーパーサークル内のプロセスの内訳を宣言することができます。
...
- **（d）の進捗アップデート：**ファシリテーターは、任意のプロジェクトを達成するか、サークル参加者の役割のいずれかの任意の責任を表すに向けた進展を強調するために、各参加者に要求します。参加者は与えられた最後のレポート以降に行われた進捗状況を共有し、プロジェクトや説明責任のない一般的なステータスがあります。各参加者は、他のサークルのメンバーは、明示的にそれが完了したか、廃棄ますどちらかまで、1が含まれていなければならないことは、特定のプロジェクトに更新を要求したが場合は、プロジェクトや説明責任がオンに報告する価値があるかを決定することができます。サブサークルのプロジェクトや責任についての進捗状況について、ファシリテーターは、リード・リンクとサブサークルの担当者リンクの両方の更新プログラムを共有する機会を許可する必要があります。
- **（e）のトリアージ問題：**ファシリテーターは、ガバナンス会議の場合と同じルールを使用して、すべての参加者から議題を勧誘することによって戦術会議で処理するように緊張の議題を構築するための責任があり、3.3節で定義されています0.4。テンションを解決するためのパスが識別されるまでしかし、戦術会議で、ファシリテーターは、単に所望のように議題項目の所有者が自分の役割と義務に他の人を従事できるようにすることで、議題項目を処理します。任意の次-アクションやプロジェクトが、この議論の中に受け入れられている場合は、長官は、それらをキャプチャし、すべての参加者にこれらの出力を配布する責任があります。ファシリテーターは、会議内のすべての議題項目を処理するための時間を確保しようとしなければならず、そうするために、残りの会議時間のその原因のシェアよりも多くを取っている任意の項目の処理を遮断することができます。
- **（f）のラウンドクロージング：**ファシリテーターは、順番に各参加者が閉会反射や会議によってトリガ他の思考を共有することができます。応答が許可されていません。
サークルは、この必要な処理を追加または変更するポリシーを採用することができます。
...
サークルの定義された役割が原因で誰かの不在に完全または部分的に不図示の戦術会議である場合は、サークルのリードリンクは、ギャップをカバーするために、その役割の中に作用することができます。鉛のリンクも存在しない場合は、任意の参加者は、その役割に代わって次世代アクションやプロジェクトを受け入れることができるが、これらは通常、ちょうど4.1.2節の条件の下で行われた要求としての役割を満たす者（bで処理することができます）。
...
組織のパートナーとして、いくつかのケースでは、この憲法のルールを破るためにも、あなたの役割、またはの権限の外に行動することが許可されます。この拡張された権限の下で働くことによって、あなたは以下の規則によって**_「個別行動"_**を取っている、とあなたがバインドされています。
...
あなただけの次のすべてに該当する個別行動を取る可能性があります。

- **（a）は**あなたがまたは全体の組織自体の、目的を果たすか、組織内のいくつかの役割の説明責任を表現するために誠実に行動しています。
- **（b）は**あなたが合理的にあなたの行動を解決したり、それは可能性が作成するよりも組織のためのより多くのテンションを防止するであろうと信じています。
- **（c）は**あなたのアクションは、原因にコミット、または支出またはそうでなければ、あなたはすでに過ごすために権限を持っているものを超えて、組織のリソースまたはその他の資産の処分を許可しませんでした。
- あなたのアクションが任意のドメインまたはポリシーに違反する場合**（D）**、あなたは合理的にあなたがすることなく、十分な長正常に必要なすべての権限を要求するために、またはあなたの行動を可能にするために、ガバナンスの変更を提案するための行動を遅らせることはできないと信じていますその潜在的価値の多くを失います。
...
個別行動を取る際に、あなたのアクションと大幅に影響を受ける可能性がある役割を満たす任意のパートナーにその背後にある意図を説明する義務があります。これらのパートナーのいずれかの要求に応じて、また、あなたの個別行動によって作成された任意の緊張を解決するのを助けるためにあらゆる合理的な追加のアクションを実行する義務があります。

あなたの個別行動が効果的に他のロール内で働く、またはドメインまたはポリシーに違反した場合、あなたは、通常、その役割、ドメイン、またはポリシーを制御誰の要求に応じて同様の個別行動を取ることを続けるのをやめる必要があります、またはリードの要求に応じてサークルのリンク影響を受けるエンティティを保持しています。
...
あなたの個別行動が既に明示的にサークルのガバナンスによってのために呼び出されていない定期的な活動やサークルが必要とする継続的な機能、およびその活性または機能のインスタンスである場合は、そのギャップを除去するために、フォローアップの手順を取るための責任があります。そのフォローアップは必要性をカバーするためにガバナンスを提案する、またはこの活動の必要性を除去するための措置を講じ含めたり、最初の場所で発生するように機能する可能性があります。
...
個別行動を取った後、あなたは、通常の作業のいずれかを行うよりも高く、このセクションで定義された当然の帰結要件をやって優先順位付けする義務を持っています。しかし、どのようなサークルのリードリンクは完全に大幅にまだこのデフォルトの優先度を変更することができ、あなたの行動の影響を受けたすべての役割が含まれています。
...
この憲法を採用することにより、Ratifiersは、そのパートナーを支配し、組織を実行するか指示するためにその権限を譲るず、もはや憲法のルールとプロセスの下でそれらに付与された権限を使用する場合を除きそのようにすることができます。しかし、この規則の例外として、Ratifiersは、彼らがそのような、または組織の内規によるそれらのコントロール外の方針によって必要と何として、委任する力を持っていないことを保持し、任意の権限を行使し続けることができます。
...
この憲法を採用すると、Ratifiersは、組織の全体的な目的を表現するための最初のサークルを確立する必要があります。この**_"アンカーサークル"_**組織で最も広いサークルになり、自動的に組織自体がコントロールするすべてのドメインを制御します。
...
Ratifiersはアンカーサークルの最初のリードリンクを任命することができます。

また、Ratifiersはリードリンクせずにアンカーサークルを離れて、リードリンクの代わりにアンカーサークルへの1つまたは複数の初期のクロスリンクを作成することができます。
...
アンカーサークル鉛リンクを持っていない場合、通常はリード・リンク権限を必要とするすべての決定は、サークルのガバナンスプロセスの有効な出力となります。サークル内の任意の役割は、このように、サークルのためのガバナンスの変化として決定を提案する第3条にプロセスとルールを使用してリードリンクの権限を行使することができます。

また、鉛のリンクとアンカーサークルで、役割の通常の権限は独裁サークルドメイン（セクション2.1.2あたり）取り消される影響を与えます。ポリシーが明示的にリードリンクの権限を行使するため、上記のとおりに、サークルのガバナンス・プロセスを使用してアクションを提案することによって、あるいは、インパクトを可能にする場合、またはその代わりに、サークルの役割は、そのドメインに影響を与える可能性があります。
...
アンカーサークルが発見し、全体的な組織の目的を表現するために自動的に責任を負います。組織の目的は、それが持続可能な世界で表現できる最も深い創造性であり、それと利用可能なすべてのものに作用する制約のすべてを与えられました。それはその歴史、電流容量、利用可能なリソース、パートナー、文字、文化、ビジネスの構造、ブランド、市場の認知度、およびその他すべての関連のリソースまたは因子を含んでいます。

アンカーサークルのリードリンクは、デフォルトでは、この責任を継承し、キャプチャし、この責任を表現する目的を更新することができます。

アンカーサークル鉛リンクを持っていない場合は、この責任はなく、自動的にアンカーサークルへの各クロスリンクに当たると、それらのいずれかは、サークルのガバナンス・プロセスを経由して更新を提案することにより、目的を更新することができます。
...
アンカーサークルのリードリンクは、サークルに名前を付け、そのドメインを明らかにし、追加したり、その説明責任を変更する権限を持っています。

そうでなければRatifiersによって指定されない限り、所望のようにアンカーサークルのリードリンクはまた、彼または彼女自身の交換を任命することができます。
...
アンカーサークルにはスーパーサークルを持っていない、と議員のリンクを選出しません。

	The Anchor Circle has no Super-Circle, and does not elect a Rep Link.

### 5.3 Initial Structure

アンカーサークルのリードLinkは、この憲法によって必要とされる通常のガバナンス・プロセスの外で、組織のための初期構造や他のガバナンスを定義することができます。その初期構造は、他のサークルが含まれている場合は、それらのサークルのリードリンクは彼らのサークル内で同じことを行うことがあります。この権限は、サークルがガバナンス・プロセスを実施し始めた前サークルから開始する初期構造を定義するために使用されてもよいです。
...
この憲法を採用する前に、組織が有効になっている任意の既存の政策や制度は、彼らがガバナンスレコードに反映されていない制約や当局を含む場合であっても、採用後の完全な力で継続。これは、補償システム、雇用および焼成プロセス、仕事関連の政策などを含むことができます

しかし、これらの従来の方針およびシステムはガバナンスが置き換えられたり、矛盾が定義されるとすぐに、すべての重量と権威を失うことになります。さらに、それらは変更されないか、または、それらの従来の形でに加えました。やることを望む誰もがそのように最初にキャプチャする必要がありますまたはそれ以外の場合は、この憲法に規定されガバナンス・プロセスを使用して、ポリシーまたはシステムに力を与えます。
...
Ratifiersまたはその後継がそれを採用するために、彼らが依拠どのような権限とプロセスを使用し、この憲法を改正またはそれを完全に廃止することがあります。改正は書面にされ、組織のすべてのパートナーがアクセスできる場所を公開しなければなりません。
